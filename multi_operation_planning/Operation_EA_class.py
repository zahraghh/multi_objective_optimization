...
class Operation_EA(Problem):
    def __init__(self,hour, G_T_now,V_wind_now,E_bat_now, electricity_demand_now, heating_demand_new,electricity_EF):
        ...
        self.electricity_prices =  float(editable_data['electricity_price'])/100 #6.8cents/kWh in Utah -->$/kWh
        if use_boilers=='yes':
            num_components +=1
            self.boiler_component = pd.read_csv(os.path.join(components_path,'boilers.csv'))
            energy_component_number['boilers']=num_components
            energy_component_type +=1
            CAP_boiler= self.boiler_component['CAP_boiler (kW)'][int(editable_data['boiler_index'])]
        else:
            CAP_boiler = 0
        if use_CHP=='yes':
            num_components +=1
            ...
            energy_component_type +=1
            CAP_CHP_elect= self.CHP_component['CAP_CHP_elect_size'][int(editable_data['CHP_index'])]
        else:
            CAP_CHP_elect = 0
        if use_solar_PV=='yes':
            energy_component_number['solar_PV']=num_components
            self.solar_component = pd.read_csv(os.path.join(components_path,'solar_PV.csv'))
            num_components +=1
        ...
        if use_wind_turbine=='yes':
            ...
            A_swept= self.wind_component['Swept Area m^2'][int(editable_data['wind_index'])]
        else:
            A_swept = 0
        ...
        if use_battery=='yes':
            ...
            CAP_battery= self.battery_component['CAP_battery (kWh)'][int(editable_data['battery_index'])]
        else:
            CAP_battery = 0
        if use_grid== 'yes':
    ...
    def evaluate(self, solution):
        ...
        self.F_boilers = self.heating_demand - self.CHP(self.CAP_CHP_elect,self.F_CHP)[1]
        self.P_grid = self.electricity_demand_new - self.CHP(self.CAP_CHP_elect,self.F_CHP)[0]
        solution.objectives[0] = self.CHP(self.CAP_CHP_elect,self.F_CHP)[3] + self.NG_boiler(self.F_boilers,self.CAP_boiler)[2] + self.P_grid*self.electricity_prices
        solution.objectives[1] = self.CHP(self.CAP_CHP_elect,self.F_CHP)[4] + self.NG_boiler(self.F_boilers,self.CAP_boiler)[3] +self.P_grid*self.electricity_EF #kg CO2
    ...
    def CHP(self,CAP_CHP_elect_size,F_CHP_size):
        ...
        UPV_maintenance = float(self.editable_data['UPV_maintenance']) #https://nvlpubs.nist.gov/nistpubs/ir/2019/NIST.IR.85-3273-34.pdf discount rate =3% page 7
        UPV_NG = float(self.editable_data['UPV_NG']) #https://nvlpubs.nist.gov/nistpubs/ir/2019/NIST.IR.85-3273-34.pdf discount rate =3% page 21 utah
        UPV_elect = float(self.editable_data['UPV_elect']) #https://nvlpubs.nist.gov/nistpubs/ir/2019/NIST.IR.85-3273-34.pdf discount rate =3% page 21 utah
        NG_prices = float(self.editable_data['price_NG'])/293.001 #Natural gas price at UoU $/kWh
        electricity_prices =  float(self.editable_data['electricity_price'])/100 #6.8cents/kWh in Utah -->$/kWh CHANGE
        lifespan_chp = int(self.CHP_component['Lifespan (year)'][0])
        ###CHP system###
        if CAP_CHP_elect_size==0:
            return 0,0,0,0,0,0,0
        else:
            index_CHP = list(self.CHP_component['CAP_CHP_elect_size']).index(CAP_CHP_elect_size)
            IC_CHP = self.CHP_component['IC_CHP'][index_CHP] #investment cost for CHP system $/kW
            CAP_CHP_Q= self.CHP_component['CAP_CHP_Q'][index_CHP] #Natural gas input mmBTU/hr, HHV
            eff_CHP_therm = self.CHP_component['eff_CHP_therm'][index_CHP] #Thermal efficiency of CHP system Q/F
            eff_CHP_elect = self.CHP_component['eff_CHP_elect'][index_CHP] #Electricity efficiency of CHP system P/F
            OM_CHP =self.CHP_component['OM_CHP'][index_CHP] #cent/kWh to $/kWh. now is $/kWh
            gamma_CHP =self.CHP_component['gamma_CHP'][index_CHP] #cent/kWh to $/kWh. now is $/kWh
    ...
    def NG_boiler(self,F_boilers,_CAP_boiler):
        ...
        index_boiler = list(self.boiler_component['CAP_boiler (kW)']).index(CAP_boiler)
        CAP_boiler = self.boiler_component['CAP_boiler (kW)'][index_boiler]
        ###Natural gas boiler###
        IC_boiler = float(self.boiler_component['Investment cost $/MBtu/hour'][index_boiler])/1000/BTUtokWh_convert #Natural gas boiler estimated capital cost of $35/MBtu/hour. now unit is $/kW
        landa_boiler = float(self.boiler_component['Variabel O&M cost ($/mmBTU)'][index_boiler])/mmBTutoBTU_convert/BTUtokWh_convert #O&M cost of input 0.95 $/mmBTU. now unit is 119 $/kWh
        gamma_boiler = float(self.boiler_component['Natural gas emission factor (kg-CO2/mmBTU)'][index_boiler])/mmBTutoBTU_convert/BTUtokWh_convert #kg-CO2/kWh is emission factor of natural gas
        eff_boiler = float(self.boiler_component['Boiler Efficiency'][index_boiler]) #efficiency of natural gas boiler
    ...
    def battery_calc(self,electricity_demand_bat,hour,E_bat_,_CAP_battery,G_T_now,V_wind_now):
        UPV_maintenance = float(self.editable_data['UPV_maintenance']) #https://nvlpubs.nist.gov/nistpubs/ir/2019/NIST.IR.85-3273-34.pdf discount rate =3% page 7
        lifespan_project = float(self.editable_data['lifespan_project']) #life span of DES
        deltat = 1 #hour for batteries
        CAP_battery = _CAP_battery
        index_battery =  list(self.battery_component['CAP_battery (kWh)']).index(CAP_battery)
        eff_bat_ch = self.battery_component['Battery efficiency charge'][index_battery]
        eff_bat_disch = self.battery_component['Battery efficiency discharge'][index_battery]
        bat_dod = self.battery_component['battery depth of discharge'][index_battery] #battery depth of discharge
        lifespan_battery = self.battery_component['Lifespan (year)'][index_battery]
        E_bat = E_bat_
        renewables_elect =  self.solar_pv_calc(self.A_solar, hour,0,G_T_now,1)[0] + self.wind_turbine_calc(self.A_swept, hour,0,V_wind_now,1)[0]
        ...
        IC_battery =  self.battery_component['Investment cost ($/kW)'][index_battery] #Battery capital investment cost is 2338 $/kW
        OM_battery = self.battery_component['Fixed O&M cost  $/kW-year'][index_battery]#fixed battery O&M cost 6$/kW-year
    ...
    def wind_turbine_calc(self,A_swept_size,hour_of_day,electricity_demand_max,V_wind_now,V_max):
        cut_in_wind_speed = self.wind_component['Cut-in Speed'][0] #2.5 m/s is the minimum wind speed to run the wind turbines
        lifespan_wind = int(self.wind_component['Lifespan (year)'][0]) #lifespan of wind turbines
        lifespan_project = float(self.editable_data['lifespan_project']) #life span of DES
        UPV_maintenance = float(self.editable_data['UPV_maintenance']) #https://nvlpubs.nist.gov/nistpubs/ir/2019/NIST.IR.85-3273-34.pdf discount rate =3% page 7
        ###Wind Turbine###
        index_wind = list(self.wind_component['Swept Area m^2']).index(A_swept_size)
        CAP_wind = self.wind_component['Rated Power kW'][index_wind]
        IC_wind = self.wind_component['Investment Cost'][index_wind] #Wind turbine capital cost in Utah 2018 1740$/kW
    ...
    def solar_pv_calc(self,A_surf_size,hour_of_day,electricity_demand_max,G_T_now,GT_max):
        lifespan_solar = int(self.solar_component['Lifespan (year)'][0]) #lifespan of solar PV System
        lifespan_project = float(self.editable_data['lifespan_project']) #life span of DES
        UPV_maintenance = float(self.editable_data['UPV_maintenance']) #https://nvlpubs.nist.gov/nistpubs/ir/2019/NIST.IR.85-3273-34.pdf discount rate =3% page 7
        ###Solar PV###
        IC_solar = self.solar_component['Investment cost ($/Wdc)'][0] #Solar PV capital investment cost is 1.75$/Wdc
        OM_solar = self.solar_component['Fixed solar PV O&M cost ($/kW-year)'][0] #fixed solar PV O&M cost 18$/kW-year
        PD_solar = self.solar_component['Power density of solar PV system W/m^2'][0] #Module power density of solar PV system W/m^2
        eff_module = self.solar_component['Module efficiency'][0] #Module efficiency
        eff_inverter = self.solar_component['Inverter efficiency'][0] #Inverter efficiency
