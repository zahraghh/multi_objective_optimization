...
def errors(path_test):
    ...
    if editable_data['Scenarios Generation/Reduction']=='yes':
        ...
        if not isinstance(editable_data['num_scenarios'], str):
            ...
            print('It indicates the number of generated scenarios using the MCMC')
        if not isinstance(editable_data['num_scenarios_intervals'], str):
            print('Please, enter "num_scenarios_intervals" value in row 32 in EditableFile.csv file (e.g., 50).')
            print('It indicates the number of intervals using the Monte Carlo simulation')
    ...
    if editable_data['Perform multi-objective optimization']=='yes':
        ...
        if not isinstance(editable_data['PV_module'], str):
            print('Please, enter "PV_module" value in row 42 in EditableFile.csv file (e.g., 1.7)')
            sys.exit()
        if not isinstance(editable_data['roof_top_area'], str):
            print('Please, enter "roof_top_area" value in row 43 in EditableFile.csv file (e.g., 8000 sqr-m)')
            sys.exit()
        if not isinstance(editable_data['population_size'], str):
            print('Please, enter "population_size" value in row 44 in EditableFile.csv file (e.g., 50)')
            sys.exit()
        if not isinstance(editable_data['num_iterations'], str):
            print('Please, enter "num_iterations" value in row 45 in EditableFile.csv file (e.g., 1000)')
            sys.exit()
        if not isinstance(editable_data['num_processors'], str):
            print('Please, enter "num_processors" value in row 46 in EditableFile.csv file (e.g., 5)')
            sys.exit()
        if not isinstance(editable_data['Boiler'], str):
            print('Please, enter "Boiler" value in row 47 in EditableFile.csv file (e.g., yes)')
            sys.exit()
        if not isinstance(editable_data['Grid'], str):
            print('Please, enter "Grid" value in row 48 in EditableFile.csv file (e.g., yes)')
            sys.exit()
        if not isinstance(editable_data['CHP'], str):
            print('Please, enter "CHP" value in row 49 in EditableFile.csv file (e.g., yes)')
            sys.exit()
        if not isinstance(editable_data['Solar_PV'], str):
            print('Please, enter "Solar_PV" value in row 50 in EditableFile.csv file (e.g., yes)')
            sys.exit()
        if not isinstance(editable_data['Wind_turbines'], str):
            print('Please, enter "Wind_turbines" value in row 51 in EditableFile.csv file (e.g., yes)')
            sys.exit()
        if not isinstance(editable_data['Battery'], str):
            print('Please, enter "Battery" value in row 52 in EditableFile.csv file (e.g., yes)')
            sys.exit()
        if not isinstance(editable_data['boiler_index'], str):
            print('Please, enter "boiler_index" value in row 53 in EditableFile.csv file (e.g., 30)')
            sys.exit()
        if not isinstance(editable_data['CHP_index'], str):
            print('Please, enter "CHP_index" value in row 54 in EditableFile.csv file (e.g., 7)')
            sys.exit()
        if not isinstance(editable_data['solar_index'], str):
            print('Please, enter "solar_index" value in row 55 in EditableFile.csv file (e.g., 1)')
            sys.exit()
        if not isinstance(editable_data['wind_index'], str):
            print('Please, enter "wind_index" value in row 56 in EditableFile.csv file (e.g., 6)')
            sys.exit()
        if not isinstance(editable_data['battery_index'], str):
            print('Please, enter "battery_index" value in row 57 in EditableFile.csv file (e.g., 30)')
            sys.exit()
